...
public class ClusterNode<T> {
    ...
    public ClusterNode(T item, ClusterNode<T> leftChild, ClusterNode<T> rightChild, double distance) {
        ...
        this.childNesting = Math.max(leftChild.childNesting,
                rightChild.childNesting) + 1;
    ...
    public int getItemIndex() {
        if (itemIndex == NO_ITEM_INDEX) {
            throw new IllegalStateException(
                    "Cluster node doesn't have an associated item");
        }
        return itemIndex;
    }
    ...
    public String toString() {
        return "ClusterNode [childNesting=" + childNesting + ", distance="
                + distance + ", item=" + item + ", itemIndex=" + itemIndex
