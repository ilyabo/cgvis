...
public class MosaicCanvas extends PCanvas {
    ...
    private static Logger logger = Logger.getLogger(MosaicCanvas.class
            .getName());
    private static final Insets NULL_INSETS = new Insets(0, 0, 0, 0);

    // private static final Cursor normalCursor = new
    // Cursor(Cursor.DEFAULT_CURSOR);
    // private static final Cursor invisibleCursor =
    // Toolkit.getDefaultToolkit().createCustomCursor(
    // new ImageIcon(new byte[0]).getImage(), new Point(0, 0), null);
    ...
    private IPointSet[] srcPointSets; // is only used for initialization
     ...
     * @param settings
     */
    public MosaicCanvas(MosaicView view, IPointSet[] pointSets,
            ViewPreferences preferences) {
        // setBackground(new Color(10, 10, 100));
        setBackground(new Color(47, 89, 134));
        // setBackground(new Color(162, 160, 149));
        setDefaultRenderQuality(PPaintContext.LOW_QUALITY_RENDERING);

        this.view = view;
        this.preferences = preferences;
        this.srcPointSets = pointSets;
//        this.colorScale = new GradientColorScale();

        final PCamera camera = getCamera();
//        camera.addChild(new ColorScaleNode(colorScale));
...
//        camera.setBounds(200, 300, camera.getWidth(), camera.getHeight());
        ...
        /* {
            ...
            protected void dragActivityFirstStep(PInputEvent aEvent) {
                ...
                if (isZooming()) {
                    if (attrsDendrogramNode != null) {
                        attrsDendrogramNode.setUseContentCache(false);
                    }
                    if (elemsDendrogramNode != null) {
                        elemsDendrogramNode.setUseContentCache(false);
                    }
                }
            }
            ...
            protected void dragActivityFinalStep(PInputEvent aEvent) {
                ...
                if (attrsDendrogramNode != null) {
                    attrsDendrogramNode.setUseContentCache(true);
                }
                if (elemsDendrogramNode != null) {
                    elemsDendrogramNode.setUseContentCache(true);
                }
            }
        }
        */;
    ...
    public void init() {
        if (srcPointSets.length == 0) {
            ...
            final IPointSet[] psets = SparseElemPointSet
                    .joinElementsByLabel(srcPointSets);
            ...
            for (int i = 0; i < psets.length; i++) {
                newPsets[i] = new PointSet(psets[i]); // create copy (to
                                                        // improve performance)
                final CGVisMosaicPlotNode mosaic = new CGVisMosaicPlotNode(newPsets[i],
                        this);
...
//        colorScale.setMinValue(minV);
//        colorScale.setMaxValue(maxV);
        ...
        for (CGVisMosaicPlotNode mosaic : mosaicNodes) {
            final PBounds b = mosaic.getBounds();
            mosaic.setBounds(b.getX() + offsetX, b.getY(), b.getWidth(), b
                    .getHeight());
            offsetX = offsetX + b.getWidth() + mosaicNodesGapX;
        }

        // Axis labels
        attrLabelsNode = new PaintedFloatingLabelsNode(true,
                createAttrsLabelIterator());
        attrLabelsNode.setVisible(showAttributeLabels);
        attrLabelsNode.setPickable(showAttributeLabels);
        elemLabelsNode = new PaintedFloatingLabelsNode(false,
                createElementsLabelIterator());
        ...
        getCamera().addChild(0, elemLabelsNode); // it's essential that
                                                // attrLabelsNode is added first
        ...
        final PropertyChangeListener fitOnceListener = new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                if (src instanceof PaintedFloatingLabelsNode) {
                    ((PaintedFloatingLabelsNode) src)
                            .removePropertyChangeListener(this);
                }
            }
        };
        if (elemLabelsNode.getVisible()) {
            elemLabelsNode.addPropertyChangeListener(
                    PaintedFloatingLabelsNode.PROPERTY_SIZE_ADJUSTED_TO_LABELS,
                    fitOnceListener);
        } else if (attrLabelsNode.getVisible()) {
            attrLabelsNode.addPropertyChangeListener(
                    PaintedFloatingLabelsNode.PROPERTY_SIZE_ADJUSTED_TO_LABELS,
                    fitOnceListener);
        }
    ...
    private LabelIterator<String> createAttrsLabelIterator() {
        return new LabelIterator<String>() {
            ...
            public boolean hasNext() {
                ...
                if (attrIndex < mosaicNodes[pointSetIndex].getPointSet()
                        .getDimension()) {
                    return true;
            ...
            public String next() {
                ...
                pos = mosaicNodes[pointSetIndex].getBoundsReference().getX()
                        + attrIndex
                        * (CGVisMosaicPlotNode.SQUARE_WIDTH + CGVisMosaicPlotNode.SPACING);
                attrIndex++;
                return label;
            }
    ...
    private LabelIterator createElementsLabelIterator() {
        return new LabelIterator() {
            ...
            public String next() {
                ...
                pos = mosaic.getBoundsReference().getY()
                        + elemIndex
                        * (CGVisMosaicPlotNode.SQUARE_HEIGHT + CGVisMosaicPlotNode.SPACING);
                elemIndex++;
                return label;
            }
        ...
        traverseFloatingPanels(new FloatingPanelVisitor() {
            public void visit(AbstractFloatingPanelNode fp) {
                if (fp.getVisible()) {
                    ...
                    if (fp.isHorizontal()) {
                        if (fpb.y <= maxVGap) {
                            insets.top = Math.max(insets.top, (int) Math
                                    .ceil(fpb.y + fpb.height));
                        } else if (fpb.y + fpb.height + maxVGap >= b.height) {
                            insets.bottom = Math.max(insets.bottom, (int) Math
                                    .ceil(fpb.height
                                            - (fpb.y + fpb.height - b.height)));
                        }
                    } else {
                        if (fpb.x <= maxHGap) {
                            insets.left = Math.max(insets.left, (int) Math
                                    .ceil(fpb.x + fpb.width));
                        } else if (fpb.x + fpb.width + maxHGap >= b.width) {
                            insets.right = Math.max(insets.right, (int) Math
                                    .ceil(fpb.width
                                            - (fpb.x + fpb.width - b.width)));
                        }
                    }
                }
            }
    ...
    public void setBounds(int x, int y, int w, int h) {
        ...
        // adjust floating panels' bounds to camera
                        ...
                        if (oldHeight > 0) {
                            ...
                            if (d <= maxVGap) {
                                fp
                                        .setBounds(
                                                fpb.x,
                                                getHeight() - fpb.height /*- (d > 0 ? d : 0)*/,
                                                fpb.width, fpb.height);
                            }
                        }
                    } else {
                        if (oldWidth > 0) {
                            ...
                            if (d <= maxHGap) {
                                fp
                                        .setBounds(
                                                getWidth() - fpb.width /*- (d > 0 ? d : 0)*/,
                                                fpb.y, fpb.width, fpb.height);
                            }
                        }
                    }
                }
    ...
    public void fitVisibleInCameraView(Rectangle2D visibleBounds,
            Rectangle2D contentBounds) {
        if (contentBounds.getWidth() <= 0 || contentBounds.getHeight() <= 0) {
            return;
        }
        final Insets insets;
        if (contentBounds.contains(visibleBounds)) { // the whole mosaic is
                                                        // inside the view
    ...
    public void fitInCameraView(boolean animate) {
        ...
        if (mb != null) {
            ...
            if (animate && preferences.useAnimation()) {
                PiccoloUtils.animateViewToPaddedBounds(getCamera(), mb, insets,
    ...
    protected void startClustering() {
        ...
        final ProgressDialog progressDlg = new ProgressDialog(getOwnerFrame(),
                "Clustering", worker, false);
    ...
    private void performClustering(ProgressTracker progress) {
        ...
        for (int i = 0; i < pointSetNum; i++) {
            ...
            // pointSet = new PointSetAttrsPerm(pointSet, attrPerm);
        ...
        camera.addChild(0, elemsDendrogramNode); // order is important
        view.elementsDendrogramAdded();

        if (getShowAttributeLabels() && attrLabelsNode.getY() == 0) {
            attrLabelsNode
                    .setY(getBounds().height - attrLabelsNode.getHeight());
        }
    ...
    class CameraInputHandler extends PBasicInputEventHandler {
        ...
        private void showCrosshair(final CGVisMosaicPlotNode mosaic, final Point cell) {
            ...
            // zoomHandler.setViewZoomPoint(null);
        }

        private void hideCrosshair() {
            crosshair.hideCrosshair();
            // final PBounds b = getMosaicBounds();
                    ...
                    if (event.isShiftDown()) {
                        if (isSelectingAttrRange()) {
                            if (mosaic == getAttrRangeSelectionStartNode()) {
                                mosaic.selectAttrRange(Math.min(cell.x,
                                        attrRangeSelectionStart), Math.max(
                                        cell.x, attrRangeSelectionStart));
                            }
                            stopSelectingAttrRange();
                        } else {
                        ...
                        if (isSelectingElemRange()) {
                            elemSelection.selectRange(Math.min(cell.y,
                                    elemRangeSelectionStart), Math.max(cell.y,
                                    elemRangeSelectionStart));
                            stopSelectingElemRange();
                        } else {
                        ...
                        view.fireElementSelectionChanged(elemSelection
                                .getSelectionUIDs());
                    }
                    repaint();
                }
